...
class AddLaptopToWishlistParams extends Equatable {
  final String laptopId;

  const AddLaptopToWishlistParams({required this.laptopId});

  @override
  List<Object?> get props => [laptopId];
}

final addLaptopToWishlistUsecaseProvider =
    Provider<AddLaptopToWishlistUsecase>((ref) {
  final wishlistRepository = ref.read(wishlistRepositoryProvider);
  return AddLaptopToWishlistUsecase(wishlistRepository: wishlistRepository);
});

class AddLaptopToWishlistUsecase
    implements UsecaseWithParams<WishlistEntity, AddLaptopToWishlistParams> {
  final IWishlistRepository _wishlistRepository;

  AddLaptopToWishlistUsecase(
      {required IWishlistRepository wishlistRepository})
      : _wishlistRepository = wishlistRepository;

  @override
  Future<Either<Failure, WishlistEntity>> call(
      AddLaptopToWishlistParams params) {
    return _wishlistRepository.addLaptop(params.laptopId);
  }
}
